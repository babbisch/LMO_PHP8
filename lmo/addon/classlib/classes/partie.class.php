...
class partie
{
    ...
    function hToreString($leer = '_', $faktor = '1')
    {
        ...
        elseif ($this->gTore == -2)
            $str = '0';  // Wenn Gast der Sieg zugesprochen wurde O Tore für Heim anzeigen
        else
            $str = intval($this->hTore) / intval($faktor);
        return $str;
    }
    ...
    function gToreString($leer = '_', $faktor = '1')
    {
        ...
        elseif ($this->gTore == -2)
            $str = '0*';  // Markieren als greenTable
        elseif ($this->hTore == -2)
            $str = '0';  // Wenn Heim der Sieg zugesprochen wurde O Tore für Gast anzeigen
        else
            $str = intval($this->gTore) / intval($faktor);
        return $str;
    }
